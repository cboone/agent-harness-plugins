...
1. **`archives.files`**: Include completions in the archive:
...
1. **`homebrew_casks:` completions field**:
...
1. Create `tap_migrations.json` in the tap repository:

   ```json
   { "PROJECT-NAME": "PROJECT-NAME" }
   ```

1. Delete old formula from tap repo after first successful cask release
1. Note: `hooks.post.install` is a **string**, not a list (documents the #178 failure)
...
1. **Pre-generated completions**: The biggest functional change. Formulas could run `generate_completions_from_executable` at install time. Casks require files to exist in the archive. Completion files are generated in `before.hooks`, included in `archives.files`, and referenced via the `completions:` field.

1. **No test block**: Casks don't support `test:` blocks. This is a functional loss but matches how Homebrew casks work.

1. **Major version bumps for add-goreleaser-homebrew and scaffold-go-cli**: The generated `.goreleaser.yml` format changes fundamentally (formula to cask). This is a breaking change.

1. **Minor version bump for setup-installers**: Detection logic adds `homebrew_casks:` recognition, but standalone formula generation is unaffected.

1. **`hooks.post.install` is a string**: Issue #178 documents real failures from using a YAML list. The template must use the string form with `|` block scalar.

1. **Standalone formulas in setup-installers unchanged**: The `Formula/PROJECT-NAME.rb` Ruby class templates are for non-GoReleaser projects. They are real formulas, not part of the GoReleaser deprecation.
...
1. `feat!: migrate scaffold-go-cli from brews to homebrew_casks` - all changes to scaffold-go-cli.md + plugin.json version bump
1. `feat: update setup-installers to detect homebrew_casks` - detection logic changes + plugin.json version bump
1. `chore: update marketplace versions for goreleaser migration` - marketplace.json version updates
...
1. Verify YAML template syntax is valid (properly indented, string-not-list for hooks)
1. Verify HOMEBREW_TAP_TOKEN sections are still in sync between add-goreleaser-homebrew and scaffold-go-cli
1. Verify all plugin.json versions match their marketplace.json entries
1. Run `check-versions` skill to confirm version consistency
