...
1. Run `check-versions` skill to verify version consistency between plugin.json and marketplace.json
1. Verify all markdown files pass markdownlint (table alignment, heading levels)
1. Verify the new cspell.md reference file follows the same structure as existing tool references
1. Check that the detection table entries in lint-and-fix SKILL.md maintain consistent column alignment
