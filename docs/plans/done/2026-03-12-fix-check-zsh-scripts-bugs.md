...
2. **`shellcheck --shell=zsh` does not exist** (#208): ShellCheck only supports sh/bash/dash/ksh. The command fails with "Unknown shell: zsh".
3. **`zcompile` invocation is broken** (#208): `zsh -c 'zcompile "$1"' -- <file>` fails because `--` is interpreted as a filename by zsh, not as an option separator.
...
2. **`plugins/check-zsh-scripts/skills/check-zsh-scripts/references/tools/shellcheck.md`**
...
3. **`plugins/check-zsh-scripts/skills/check-zsh-scripts/references/tools/zcompile.md`**
   - Line 10: Fix to `zsh -c 'zcompile "$1"' _ <file>`
   - Add note explaining `_` is the conventional `$0` placeholder

4. **`plugins/check-zsh-scripts/README.md`**
...
2. **`plugins/check-zsh-scripts/skills/check-zsh-scripts/references/tools/beautysh.md`**
   - Delete this file

3. **`plugins/check-zsh-scripts/skills/check-zsh-scripts/references/tools/shfmt.md`**
...
2. Confirm zcompile command works: create a temp zsh file and run `zsh -c 'zcompile "$1"' _ /tmp/test.zsh`
3. Grep for any remaining "beautysh" references: `grep -r beautysh plugins/check-zsh-scripts/`
4. Grep for remaining `--shell=zsh`: `grep -r 'shell=zsh' plugins/check-zsh-scripts/`
5. Grep for old zcompile pattern: `grep -r "zcompile.*-- " plugins/check-zsh-scripts/`
6. Run `/check-versions` skill to verify plugin.json and marketplace.json versions match
