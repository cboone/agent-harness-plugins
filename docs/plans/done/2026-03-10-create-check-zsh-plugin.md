...
- **Workflow** (numbered steps):
  ...
  2. **Check Tool Availability** - `command -v` for each tool. Report missing tools with install commands. Minimum: `zsh` must be available.
  3. **Run Tools** - Run sequentially in this order, capturing stdout/stderr/exit code:
  ...
  4. **Report Results** - Summary table (tool, status, issue count, filtered false positives), then detailed findings by file.
  5. **Fix Issues** - `beautysh <file>` for auto-formatting. For other tools, present findings with manual fix guidance from reference docs. Re-run `zsh -n` after fixes to verify no new syntax errors.
  6. **Clean Up** - Remove any `.zwc` artifacts.
...
2. `plugin.json` version (`1.0.0`) matches `marketplace.json` entry version
3. `marketplace.json` `metadata.version` is `1.24.0`
4. `marketplace.json` entry alphabetically between `bootstrap-project` and `clean-up-agent-config`
5. SKILL.md frontmatter has only `name` and `description` fields
6. All 7 reference files exist in `references/tools/`
7. README ToC has `[Check Zsh](#check-zsh)` before `[Handle Secrets](#handle-secrets)` in Code Quality
8. README description section has `#### Check Zsh` before `#### Handle Secrets`
9. CLAUDE.md tree has `check-zsh/` between `bootstrap-project/` and `commit/`
10. Run `check-versions` skill to validate consistency
