...
- **Gitleaks**: fast pattern matching on every push and PR (current behavior, with a permissions fix from issue #163)
- **TruffleHog**: deeper verification-based scanning on pushes to main (post-merge) and a weekly schedule

This is a breaking change (renamed plugin, new trigger), so the plugin version becomes 2.0.0.

This also fixes [issue #163](https://github.com/cboone/cboone-cc-plugins/issues/163): the gitleaks templates currently have no `permissions` block, causing 403 errors on repos with restricted default permissions. Both gitleaks templates will add explicit `permissions: contents: read` and `pull-requests: read`.
