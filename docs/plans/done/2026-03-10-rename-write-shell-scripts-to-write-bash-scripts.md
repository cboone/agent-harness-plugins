...
2. `grep -r "write-shell-scripts" .` should return zero results (excluding `docs/plans/done/`)
3. Run `/check-versions` skill to verify plugin.json and marketplace.json versions match
4. Confirm the skill directory structure is correct: `plugins/write-bash-scripts/skills/write-bash-scripts/SKILL.md`
