...
- Immediately before any diff or commit-log read, run `git -c core.hooksPath=/dev/null fetch --no-recurse-submodules -- "$remote" "refs/heads/$base_branch:refs/remotes/$remote/$base_branch"` and compare `git rev-parse "$base_ref"` with the recorded snapshot. If it changed, restart before reading review content.
...
- On a re-review, read the complete allowlisted tree delta and commit metadata, then read each commit's allowed-path patch against every parent. Include merge resolutions, classify imported and base changes using those patches, and disclose uncertain attribution. Keep established rename pairs together; never include excluded paths. Use recorded SHAs for every evidence read, reserving remote-tracking refs for refresh comparisons. Capture all dynamic arguments as quoted data.
