...
- Immediately before any diff or commit-log read, run `git -c core.hooksPath=/dev/null fetch "$remote" "$base_branch:refs/remotes/$remote/$base_branch"` and compare `git rev-parse "$base_ref"` with the recorded snapshot. If it changed, restart synchronization before reading review content.
- Run `git --no-pager diff --no-color --no-ext-diff --no-textconv --stat "$base_ref...$head_sha"`, then the matching full diff and `git --no-pager log --no-color --no-merges --format='%h %an %s%n%n%b' "$base_ref..$head_sha"`. On a re-review, use `git --no-pager log --no-color --no-merges --format='%H %s' "$last_review_sha..$head_sha" --not "$base_ref"` and `git --no-pager show --no-color --no-ext-diff --no-textconv --format= "$commit_sha"` so base-branch commits merged after the last review are excluded. Capture remote and branch names as data, never interpolate them into shell source.
...
1. **Shell-safe refs.** Use remote names and base branch names containing shell metacharacters where Git permits them. Confirm fetch, refresh, diff, log, and revision commands use quoted variable arguments and cannot execute shell syntax. Also supply a `--since` ref containing a quote and shell metacharacters; confirm it reaches `git rev-parse --verify --end-of-options` as data and cannot execute shell syntax.
