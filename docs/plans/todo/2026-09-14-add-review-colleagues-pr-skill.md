...
- **Thin requirements:** if the title and body are empty or only unfilled template text, no linked issue states substantive intent, and no external doc was supplied, ask before reviewing. A substantive title can establish intent even when the body is empty.
...
1. **Thin requirements.** On a PR with an empty or template-only title and body and no substantive linked issue or external doc, the skill asks before reading code. Also confirm that a substantive title with an empty body passes this gate.
