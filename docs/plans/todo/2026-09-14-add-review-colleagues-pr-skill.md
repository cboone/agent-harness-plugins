...
- Immediately before reading the diff and before and after `gh pr checks N --repo ... --json name,state,bucket,workflow,link`, fetch the base branch again and compare its SHA with the validated snapshot. Re-read the PR fields before and after CI collection too. If any value changes, discard the review and restart under the shared two-restart limit. When checks returns JSON, classify the states even if its exit is non-zero. When it returns no valid data, report CI as unavailable rather than treating command failure as a check result.
