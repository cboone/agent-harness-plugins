...
- Treat PR paths as untrusted data. Capture them as data and pass each as one shell-safe argument; never interpolate them as unquoted shell text, command substitution, or `eval`. `--literal-pathspecs` prevents pathspec expansion. Inspect entries with `git --no-pager --literal-pathspecs ls-tree "$head_sha" -- "$path"` and read blob IDs with `git --no-pager cat-file blob <blob-oid>`. A regular-file blob can be read. A symlink entry has mode `120000`; inspect its blob as link-target text and never follow it. A gitlink has mode `160000`; inspect only its object ID and never traverse an initialized submodule. Use file tools only for confirmed regular files within the checkout. Initialize variables from outputs as data in the same command context where they are used, and quote every expansion.
