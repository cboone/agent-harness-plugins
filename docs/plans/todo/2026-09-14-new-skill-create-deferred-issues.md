...
| Composition with `pr`  | A small `pr` change in the same PR: a `## Follow-ups` section (no closing keyword) listing issues filed earlier in the session for this branch, and those numbers excluded from issue detection. `pr` goes 1.8.5 to 1.9.0           |
...
1. **Establish context** (read-only).
   - **Repository.** List remote names with `git remote`, then sanitize each remote URL before any output or use. Normalize URI authorities without losing ports; convert SCP-style SSH remotes to a host-qualified selector; resolve SSH aliases with `ssh -G`; then use `gh repo view <selector> --json nameWithOwner,visibility,isArchived,isFork,parent,hasIssuesEnabled,defaultBranchRef`. Do not fall back to a host-free `owner/name` when a host or alias cannot be established. On a host other than `github.com`, use `HOST/OWNER/NAME` for `--repo` and `--hostname HOST` for `gh api`. `parent` carries no `nameWithOwner`; build it from `.parent.owner.login` and `.parent.name`. Resolve an `upstream` remote the same way to identify third-party destinations. With no `origin`, every item's target starts unresolved.
...
- **Title**: imperative, under 70 characters, no trailing period. Pass the approved title as one literal argument. Encode an apostrophe as `'\''` inside the shell argument; never rephrase an approved title.
...
- `.claude-plugin/marketplace.json`: the new entry between `commit` and `create-issue`, `"category": "issues-and-worktrees"`, `"source": "./plugins/create-deferred-issues"`. `metadata.version` from `bin/compute-catalog-state` is `catalog-M71-m108-p160-n58` after merging `main`, confirmed from the script rather than assumed.
