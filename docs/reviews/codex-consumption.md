...
**Milestone 1: declarations resolved; preflight adoption pending.** Thirteen skills declare their required and optional dependencies, and `bin/check-cross-references` rejects an undeclared composition and a malformed, unresolved, repeated, or unused declaration. The adapter contract defines the preflight, the missing-dependency stop, and the skip-and-report behavior for selection-driven candidates. Every optional dependency now has its fallback in the workflow body, following Copilot's review of PR #468. The required-dependency preflight is not yet applied anywhere. Milestone 2 adds it to the Git and review chain: `pr` and `resolve-copilot-pr-feedback` before they invoke `lint-and-fix`, and `monitor-pr` before its first sync or repair through `merge-main`, `lint-and-fix`, or `resolve-copilot-pr-feedback`. Milestone 3 adds it to `pin-everything`, `review-dependabot-config`, and `upgrade-everything`, each of which requires `lint-and-fix`.
