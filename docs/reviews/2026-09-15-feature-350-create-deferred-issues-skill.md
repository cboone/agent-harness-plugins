...
- Catalog state changes from `catalog-M70-m103-p156-n57` to `catalog-M71-m104-p152-n58`. The computed value matches; plugin validation confirms catalog agreement and generated-tree freshness. The lower patch sum is expected when `pr` advances to a minor version with patch zero.
...
- Version check: the new plugin remains `1.0.0`; `pr` remains `1.9.0`; catalog state recomputes to `catalog-M71-m104-p152-n58`. No additional version bump is needed for corrections to the new plugin before its first release.
