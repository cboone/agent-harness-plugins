...
**Deviations**: one addition beyond the plan. `cmd_check` now rejects an option-shaped resource name, matching `cmd_claim`. This was found in review, not planned. It is in scope because it closes a silent-failure path in the exact call the skills make.

**Fidelity**: the advisory-not-a-lock constraint is honored at every layer. `claim` records over a live holder rather than refusing, both skills are instructed never to refuse, and `suggest-next-issue` flags rather than hides unless `--parallel-only` is passed. Nothing in the implementation quietly turns the claim into enforcement.
