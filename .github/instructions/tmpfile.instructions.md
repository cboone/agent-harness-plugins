...
- **`mktemp -u "${TMPDIR:-/tmp}/<prefix>-XXXXXX"` is the intended form**, for files, as is `mktemp -d "${TMPDIR:-/tmp}/<prefix>-XXXXXX"` for directories. A file already written this way needs no further change; do not report it as an unresolved gap. Only a bare `mktemp`, a bare `mktemp -d`, a `mktemp -ut <prefix>`, or a literal `/tmp/...` template is a finding, because each of those selects a directory the sandbox denies.
