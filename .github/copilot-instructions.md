...
- **Scaffold templates reflect the owner's repo conventions**: The scaffold-new-repo skill templates (gitignore entries, project type inference) are derived from analyzing 50+ of the owner's repositories. Patterns like `*.code-workspace` in Ruby gitignore are intentional conventions from the owner's actual projects, not generic best practices. Do not suggest replacing them with standard language templates.
