...
1. **Plant an error the analyzer must reject** in each declaration you believe is covered, one at a time, and confirm something fails. A type error is the sharpest form and assumes a compile or type-check step over declarations. Where there is none, as in an untyped dynamic language, substitute whatever does analyse at that granularity: a type checker run in strict mode, an import-time failure, a linter rule that resolves names, or a deliberate syntax error, which every parser rejects whether or not anything calls the code. A clean result means nothing is analysing that declaration, and if no analyser reaches it at all then this step cannot discriminate and step 3 has to carry the work.
