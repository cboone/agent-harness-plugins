...
1. Render with `--standalone` to a temporary path rather than onto the board, creating the board directory if it does not exist.
2. On a re-sync, look for the previous board at that exact path and run `report-board compare` against it before installing anything. Stop here if the report names a changed `title`, `repo`, or `repoUrl`: the board has moved, and installing would strand it.
3. Immediately before installing, extract the board at that path again and confirm it still holds the data this sync started from. A difference means another session published while this one was gathering, so begin again from the newer board instead of overwriting it.
4. Move the rendered file onto the board path, then give the user the absolute path; the file opens in any browser.
