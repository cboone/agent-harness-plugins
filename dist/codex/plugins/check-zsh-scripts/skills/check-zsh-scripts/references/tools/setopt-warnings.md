...
- **CI compatibility**: The `set-up-ci` zsh CI workflow template sets `SKIP_SETOPT_CHECK: "1"` in the job `env` to keep lint jobs purely static analysis. Generated check scripts must honor this env var to interoperate.
