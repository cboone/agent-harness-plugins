...
1. **Replace `reviewers` with code owners** when that fix was selected, in the same local change. Find the code owners file (`.github/CODEOWNERS`, `CODEOWNERS`, or `docs/CODEOWNERS`, in that order; create `.github/CODEOWNERS` when none exists), add an entry that assigns the former reviewers to every path the entry's updates change: its manifests and lockfiles, and for `github-actions` the workflow files and each composite action's `action.yml` or `action.yaml`, and show `git diff` for both files. Remove `reviewers` from the config only once the code owners entries are written, so review coverage never lapses.
