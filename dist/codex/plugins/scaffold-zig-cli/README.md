...
This covers every command the workflow runs, not only the Zig ones: `date +%Y` for the LICENSE year, `gh release view` and `gh api` for the `cboone/gh-actions` SHA refresh, and `mkdir -p` plus `touch` for the directory stubs.
