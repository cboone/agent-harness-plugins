...
{
  "permissions": {
    "allow": ["Bash(git init*)", "Bash(git add *)", "Bash(git commit *)", "Bash(git config *)", "Bash(git -C *)", "Bash(git status*)", "Bash(gh api user*)", "Bash(gh api \"repos/*)", "Bash(gh release view*)", "Bash(zig build*)", "Bash(zig fmt*)", "Bash(zig version)", "Bash(date +%Y)", "Bash(echo *)", "Bash(ls -d *)", "Bash(mkdir -p*)", "Bash(touch *)"]
  }
}
```

This covers every command the workflow runs, not only the Zig ones: `git -C` for the step 4 identity re-read in the target, `ls -d` for the step 3 overwrite preflight, `echo` for the SHA-refresh output, `date +%Y` for the LICENSE year, `gh release view` and `gh api` for the `cboone/gh-actions` SHA refresh, and `mkdir -p` plus `touch` for the directory stubs.
