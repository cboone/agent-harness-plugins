...
- **npm**: Use `actions/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7.0.0` with `cache: "npm"`. No extra setup needed.
- **yarn** (detected via `.yarnrc.yml` or `yarn.lock`): Add a `run: corepack enable` step before `actions/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7.0.0`. Use `cache: "yarn"`.
- **pnpm** (detected via `pnpm-lock.yaml`): Add a `run: corepack enable` step before `actions/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7.0.0`. Use `cache: "pnpm"`.
- **bun** (detected via `bun.lock`): Replace the `actions/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7.0.0` step entirely with `oven-sh/setup-bun@0c5077e51419868618aeaa5fe8019c62421857d6 # v2.2.0` (omit the `cache` and `node-version` parameters; Bun manages its own caching). The `actions/setup-node` `cache` option does not support Bun.
...
- For **bun**: replace `actions/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7.0.0` with `oven-sh/setup-bun@0c5077e51419868618aeaa5fe8019c62421857d6 # v2.2.0` and remove `node-version` and `cache` parameters
