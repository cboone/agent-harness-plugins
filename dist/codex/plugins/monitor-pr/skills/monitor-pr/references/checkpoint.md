...
1. Resume step 4 dispatch with the restored state. A review already requested for this head is not requested again. A review already processed is not processed again. If the post-request quiet count reaches two without a review, or the feedback probe still reports open threads or format drift against an already processed review, escalate under the existing rules. A review-body finding count that stays above zero against an already processed review is expected, not an escalation.
