...
1. **Copilot reviewed the current head, step 7b already processed that review, and this tick has no new input to give it**: escalate per step 9. Two shapes arrive here and both are terminal for that review. Either the outcome was something other than `Completed` or `No unresolved Copilot feedback`, so it never clears the axis; or the outcome was one of those two and the probe still reports open threads or format drift, which is the case step 7b calls out as unable to change on its own. The `id` guard in the condition above will not send the review to step 7b a second time, and no later tick alters an immutable review, so without this condition the tick falls through to the wait below and the watch never ends. **A review-body finding alone never arrives here**, because a successful outcome clears it by record rather than by a later probe, per the Ready Criteria rules.
