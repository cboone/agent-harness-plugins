...
- **`hasFormatDrift`**: the complete drift signal. It is true when an old suppressed section contains no parseable findings, or when a `ccr-overview-v2` review has no parseable findings and either states a finding count above zero or pairs a non-clean verdict with no section the parser recognizes. Threads listed under `Open (N)` are reported by `fetch`, and entries under `Resolved since last review (N)` were closed in an earlier round, so neither is a finding this command missed. A stated count above zero still reports drift when a resolved section is present, because the count covers open threads and `Previously missed` entries and so records a shortfall the parser must explain. `**Findings:** None` does not clear drift on its own: the count omits the line-less findings appended to file-summary cells, so it cannot rule out an unparsed layout.
