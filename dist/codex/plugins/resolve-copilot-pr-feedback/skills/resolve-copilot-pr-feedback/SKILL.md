...
- **`hasFormatDrift`**: the complete drift signal. It is true when an old suppressed section contains no parseable findings, or when a `ccr-overview-v2` review has no parseable findings and either falls short of its own stated count or pairs a non-clean verdict that nothing accounts for.
  - **Shortfall**: the `**Findings:**` line states how many inline threads Copilot opened, as a severity breakdown whose counts are summed (`2 <medium> · 1 <low>` is three). A total above the number announced under `Open (N)`, or a value reading as neither `None` nor a breakdown, leaves findings this command cannot explain. A shortfall reports drift whatever else the body contains, including on an approval and including when some findings did parse.
