...
{
  "permissions": {
    "allow": ["Bash(git init*)", "Bash(git add *)", "Bash(git commit *)", "Bash(git config *)", "Bash(git remote *)", "Bash(gh api user*)", "Bash(gh repo view*)", "Bash(gh issue create *)", "Bash(go build*)", "Bash(go get *)", "Bash(go mod *)", "Bash(mktemp -u \"${TMPDIR:-/tmp}/gh-issue-body-*\")", "Bash(rm -f *gh-issue-body-*)"]
  }
}
```
