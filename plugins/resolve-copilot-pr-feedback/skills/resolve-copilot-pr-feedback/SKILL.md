...
- **`hasFormatDrift`**: the complete drift signal. It is true when an old suppressed section contains no parseable findings, or when a `ccr-overview-v2` review has no parseable findings and either falls short of its own stated count or pairs a non-clean verdict that nothing accounts for.
  - **Shortfall**: the `**Findings:**` line states how many inline threads Copilot opened, so a count above the number announced under `Open (N)`, or a count line reading as neither `None` nor a number, leaves findings this command cannot explain. A shortfall reports drift whatever else the body contains, including on an approval.
