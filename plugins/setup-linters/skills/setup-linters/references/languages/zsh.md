...
On Ubuntu CI, use `apt-get install devscripts` for checkbashisms, `cargo install --locked --version 4.3.1 shellharden` for shellharden, and the `mfinelli/setup-shfmt@v4` action for shfmt. ShellCheck is pre-installed on `ubuntu-latest`.
