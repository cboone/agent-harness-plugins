...
Scan for language and file-type markers using Glob. **Exclude dependency directories from all searches** to avoid false positives from vendored code: `node_modules/`, `.yarn/`, `.lake/` (Lean dependency/build directory, including `.lake/packages/**`), and any others the project carries.
