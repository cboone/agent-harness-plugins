...
- The three helpers in `create-worktree/scripts/` and `address-issue-in-worktree/scripts/` must stay byte-identical, as must `resolve-copilot-threads` in `resolve-copilot-pr-feedback/scripts/` and `monitor-pr/scripts/`. Update each pair and its scrut coverage together.
