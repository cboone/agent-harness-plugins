...
- Each plugin has `.claude-plugin/plugin.json`, a README, and a skill or hook entry point. Canonical metadata lives in `../.claude-plugin/marketplace.json`; update registration and descriptions together. The plugin manifest is the sole version source.
