...
- **Use immutable evidence.** After recording the fetched snapshot, use `base_sha` and `head_sha` for every ancestry check, diff, attribute source, tree lookup, and governing-file read. Use `base_ref` only to fetch and verify that the base has not changed. Initialize recorded SHAs as data in every command context.
