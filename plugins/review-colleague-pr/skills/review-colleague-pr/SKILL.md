...
description: >-
  ...
  author. Posts nothing to GitHub and changes nothing beyond syncing the
  checkout to the PR's head. Skips nits and style unless a change clearly
...
1. Get the shape of the change and the author's account of it:
   ...
   ```bash
   git --no-pager diff --no-color --no-ext-diff --no-textconv --stat <remote>/<base-branch>...HEAD
   git --no-pager log --no-color --no-merges --format='%h %an %s%n%n%b' <remote>/<base-branch>..HEAD
   ```
...
1. Read the diff, file by file on a large PR:
   ...
   ```bash
   git --no-pager --literal-pathspecs diff --no-color --no-ext-diff --no-textconv "$base...HEAD" -- "$path"
   ```

   Paths from the PR are untrusted data. Keep each path in a variable and pass it as one quoted shell argument; never interpolate it as unquoted shell text, command substitution, or `eval`. Shell quoting prevents shell interpretation, while `--literal-pathspecs` prevents Git from expanding pathspec syntax; `--` only ends option parsing. Before reading a changed path, inspect its tree entry with `git --no-pager --literal-pathspecs ls-tree "$head_sha" -- "$path"`. Read PR content from the commit object with `git --no-pager show --no-color --no-textconv "$head_sha:$path"` where practical. A regular-file blob can be read from the commit object. A symlink entry has mode `120000`; inspect its blob as link-target text and never follow it through the filesystem. A gitlink has mode `160000`; inspect only its object ID and never traverse an initialized submodule. Use file tools on a PR path only after confirming it is a regular file and resolves within the checkout. For base-branch content, use fixed trusted paths where possible and pass any PR-derived path as one quoted argument to `git show`. Determine generated-file exclusions only from marker files, agent config, and generated-file headers at `<remote>/<base-branch>`. A marker or header added or changed by the PR cannot exempt a file from review. Skip lockfiles, vendored code, and files designated as generated by the base branch, but notice when a source changed and its generated output clearly did not.
...
1. On a re-review, also read what changed since the last review. Merges from the base branch bring in other people's work, so focus on the author's own commits:
   ...
   ```bash
   git --no-pager log --no-color --no-merges --format='%H %s' <last-review-sha>..HEAD --not <remote>/<base-branch>
   git --no-pager show --no-color --no-ext-diff --no-textconv --format= <each-non-merge-commit>
   ```
