...
1. **Copilot reviewed the current head and step 7b already processed that review with an outcome other than `Completed` or `No unresolved Copilot feedback`**: escalate per step 9. Nothing further can move: the outcome does not clear the axis, and the `id` guard in the condition above will not send the review to step 7b a second time. Without this condition the tick falls through to the wait below and the watch never ends.
