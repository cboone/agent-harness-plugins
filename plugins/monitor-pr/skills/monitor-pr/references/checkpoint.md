...
| Snapshot           | All four readiness axes, individual check states, latest Copilot review ID and SHA, draft state, `mergeable`, `mergeStateStatus`, and informational `reviewDecision` |
...
1. Resume step 4 dispatch with the restored state. A review already requested for this head is not requested again. A review already processed is not processed again. If the post-request quiet count reaches two without a review, or findings remain against an already processed review, escalate under the existing rules.
