...
- [`workmux`](https://github.com/raine/workmux) 0.1.233 or newer, always. `workmux add --dry-run` arrived in 0.1.220 and slash-separated generated names such as `fix/issue-123` stopped being flattened in 0.1.233; the branch naming here needs both. Verified against 0.1.261. Older versions degrade in two different ways rather than one: below 0.1.220 the dry run fails outright and the skill falls back to deriving the name from the issue title, while 0.1.220 through 0.1.232 generate a name successfully but flatten the prefix, so `fix/auth-timeout` silently becomes `fix-auth-timeout` and no fallback is triggered. Passing an explicit branch name skips naming altogether and works on any version.
