...
Capacity follows the lane's mode: one branch at a time for a serial or head lane, and every unblocked issue at once for an any-order lane. A serial lane runs its first issue that can start, and a head lane only its head, so a head that cannot start holds its whole lane at zero. A head lane also shows how many issues its head frees. It shows that count only while the head is the branch the lane is running, so a lane whose slot is held by another branch already in progress promises nothing, and the count covers only what finishes alongside the head: a rider kept back by a blocker of its own frees nothing. "Branches at once" in the header is the sum across lanes. A branch carrying an issue that is better after another open issue counts as queued rather than runnable, so it adds nothing to capacity until its target lands, unless its work has already started.
