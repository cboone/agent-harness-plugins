...
- **Review checklists**: A style guide whose rules a pull request reviewer can check may add `references/review-checklist.md`, which `set-up-review-config` installs for automated reviewers. It follows the "Review checklists" format in `docs/plugin-development.md` and needs an entry in `set-up-review-config`'s `references/guides.md`.
