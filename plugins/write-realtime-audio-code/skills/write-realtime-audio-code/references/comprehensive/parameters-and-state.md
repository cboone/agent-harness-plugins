...
CLAP latency changes during activation, and its `changed` notification is limited to that phase. Structural ports and critical parameter changes requiring `CLAP_PARAM_RESCAN_ALL` are legal only while deactivated. They include adding or removing parameters and changing an existing parameter's behavior flags, range, or cookie. Use value, text, and info rescan flags for cache or presentation changes that do not require a structural rescan. A host's `request_restart` can be delayed, so continue with the current valid configuration without waiting in the callback or reporting the pending configuration as active. Update host-visible caches and notifications only at permitted boundaries.
