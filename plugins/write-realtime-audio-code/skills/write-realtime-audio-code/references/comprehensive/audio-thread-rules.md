...
`reset` clears only the active 256-frame delay window, so its work has a stated 256-sample bound. A rejected worker request processes the same work on the callback only if that fallback also fits the documented bound; otherwise the prepared last-valid result is used. `stop_processing` marks the instance not processing; it remains active until `deactivate`. `deactivate` runs on the main thread, waits for worker completion, then releases scratch storage. A focused adapter test supplies aliased stereo input, one input with three outputs, a constant channel, a 256-frame block, and a 257-frame block. It establishes buffer and lifecycle behavior for this declared configuration, not other formats or hosts.
