...
A `## Contents` section sits between the intro paragraph and `## Install`. It is section-level navigation: one bullet per H2, with the `Install` and `Skills` bullets naming their H3s inline. It never lists individual plugins, so adding a plugin does not touch it. Update it only when an H2 or a skills category is added, renamed, or removed, and keep every anchor resolvable, because markdownlint's MD051 checks them.
